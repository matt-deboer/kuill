import { types } from '../actions/access'
import yaml from 'js-yaml'
import queryString from 'query-string'
import { LOCATION_CHANGE } from 'react-router-redux'
import { arraysEqual, keyForResource, statusForResource } from '../../comparators'
...
function applyFiltersToResource(filters, resource) {
  ...
  for (var field in filters) {
    ...
    if (field === '*') {
      ...
      if (resource.kind === 'RoleBinding' || resource.kind === 'ClusterRoleBinding') {
        for (let subject of resource.subjects) {
          if (subject.name in values) {
            matched = true
            break
          }
        }
        if (resource.roleRef.name in values) {
          matched = true
          break
        }
      }
    ...
    if ((resource.metadata[field] in values)
    ...
    if (field === 'subject' && (resource.kind === 'RoleBinding' || resource.kind === 'ClusterRoleBinding')) {
      for (let subject of resource.subjects) {
        if (subject.name in values) {
          resource.isFiltered = false
          return
    ...
    if (field === 'role' && (resource.kind === 'RoleBinding' || resource.kind === 'ClusterRoleBinding')) {
      if (resource.roleRef.name in values) {
        resource.isFiltered = false
        return
      }
